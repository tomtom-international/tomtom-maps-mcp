...
/** Polygon features from a dataset, for `within.dataset_ids`. */
const polygonsOf = (data: unknown): { type: string; coordinates: unknown }[] => {
  const features = (data as { features?: unknown[] })?.features ?? [];
  const out: { type: string; coordinates: unknown }[] = [];
  for (const feature of features) {
    const geometry = (feature as { geometry?: { type?: string; coordinates?: unknown } })?.geometry;
    if (geometry?.type === "Polygon" || geometry?.type === "MultiPolygon") {
      out.push({ type: geometry.type, coordinates: geometry.coordinates });
    }
  }
  return out;
};

/** True when a dataset's first feature is a line — i.e. it holds a route. */
const holdsRouteGeometry = (data: unknown): boolean => {
  const features = (data as { features?: unknown[] })?.features ?? [];
  const type = (features[0] as { geometry?: { type?: string } })?.geometry?.type;
...
export async function resolveWithin(
  ...
  for (const id of where.dataset_ids ?? []) {
    const dataset = getDataset(id);
    if (!dataset) {
      return {
        error:
          `Dataset "${id}" is not available (datasets live ${datasetLifetimePhrase()}). Re-run the tool that ` +
          "produced it, or name the area with `queries` instead.",
      };
    }
    const polygons = polygonsOf(dataset.data);
    if (polygons.length === 0) {
      ...
      if (holdsRouteGeometry(dataset.data)) {
        const corridor = await bufferRoute(dataset.data, IMPLIED_ROUTE_CORRIDOR_METERS);
        if ("error" in corridor) return corridor;
        areas.push(corridor.value);
        continue;
      }

      const bbox = bboxOf(dataset.data);
      if (!bbox) {
        return {
          error:
            `Dataset "${id}" holds no polygon or bounding box to search within. Use a dataset ` +
            "from tomtom-find-reachable-areas, or name the area instead.",
        };
      }
      areas.push({ bbox, source: "dataset" });
      continue;
    }
    for (const polygon of polygons) areas.push({ polygon, source: "dataset" });
  }

  if (where.route) {
    const dataset = getDataset(where.route.dataset_id);
    if (!dataset) {
      return {
        error:
          `Route dataset "${where.route.dataset_id}" is not available. Re-run tomtom-plan-route ` +
          "and pass the new dataset_id.",
      };
    }
    const corridor = await bufferRoute(dataset.data, where.route.widthMeters);
    if ("error" in corridor) return corridor;
    areas.push(corridor.value);
  }
...
/** Buffers a route's line geometry into a search corridor. */
async function bufferRoute(
  data: unknown,
  widthMeters: number
): Promise<{ value: ResolvedArea } | { error: string }> {
  const feature = (data as { features?: unknown[] })?.features?.[0];
  if (!feature) return { error: "That route dataset holds no route geometry." };

  // Imported lazily: turf is large, and only the corridor path needs it.
  const turf = await import("@turf/turf");
  try {
    const buffered = turf.buffer(feature as never, widthMeters / 2, { units: "meters" });
    const geometry = (buffered as { geometry?: { type?: string; coordinates?: unknown } })
      ?.geometry;
    if (!geometry?.type) return { error: "Could not build a corridor from that route." };
    return {
      value: {
        polygon: { type: geometry.type, coordinates: geometry.coordinates },
        source: "route",
    ...
    const message = caught instanceof Error ? caught.message : String(caught);
    return { error: `Could not build a corridor from that route: ${message}` };
  }
}
...
export async function resolveNearby(
  ...
  if (where.dataset_id) {
    const dataset = getDataset(where.dataset_id);
    const feature = (dataset?.data as { features?: unknown[] })?.features?.[0];
    const coords = (feature as { geometry?: { coordinates?: unknown } })?.geometry?.coordinates;
    if (Array.isArray(coords) && typeof coords[0] === "number") {
      return { value: { position: coords as Position, radiusMeters } };
    }
    ...
    return {
      error:
        `Dataset "${where.dataset_id}" has no feature with usable coordinates to search around ` +
        `(datasets live ${datasetLifetimePhrase()}). Give \`where.position\` instead, or re-run ` +
