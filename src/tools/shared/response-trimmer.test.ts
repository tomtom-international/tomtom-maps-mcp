...
describe("buildCompressedResponse", () => {
  const dataset = { kind: "places" as const, provenance: { tool: "tomtom-test", params: {} } };

  it("stores the full data and returns its dataset_id", async () => {
    const trimmedData = { summary: { query: "test" } };
    const fullData = { summary: { query: "test", queryTime: 42 }, results: [] };

    const response = await buildCompressedResponse(trimmedData, fullData, true, true, dataset);
    ...
    expect(parsed._meta.dataset_id).toMatch(/^ds_[0-9a-f]{16}$/);
  });

  it("still stores when show_ui is false", async () => {
    // Changed deliberately: the app is no longer the only consumer of a dataset.
    // describe-dataset (and, from phase 2, analyse-data) need the full payload
    // whether or not anything is being drawn — that is the point of holding it.
    const response = await buildCompressedResponse({ a: 1 }, { a: 1, b: 2 }, false, true, dataset);

    const parsed = JSON.parse(response.content[0].text);
    expect(parsed._meta.show_ui).toBe(false);
    expect(parsed._meta.dataset_id).toMatch(/^ds_/);
  });

  it("skips the store when no provenance is supplied", async () => {
    // A caller that stores its own dataset (dynamic-map) passes no envelope here.
    const response = await buildCompressedResponse({ a: 1 }, { a: 1, b: 2 }, true);

    const parsed = JSON.parse(response.content[0].text);
    expect(parsed._meta.show_ui).toBe(true);
    expect(parsed._meta.dataset_id).toBeUndefined();
  });

  it("should default to show_ui true", async () => {
    const response = await buildCompressedResponse(
      { data: "test" },
      { data: "test" },
      undefined,
      true,
    ...
    const parsed = JSON.parse(response.content[0].text);
    expect(parsed._meta.show_ui).toBe(true);
    expect(parsed._meta.dataset_id).toBeDefined();
  });
