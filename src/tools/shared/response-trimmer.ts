 ...
 * pass it to `tomtom-describe-dataset` (and, from phase 2, `tomtom-analyse-data`).
...
export async function buildCompressedResponse<T>(
  ...
  // dataset now. `describe-dataset` and (phase 2) `analyse-data` need the full
  // response whether or not anything is being drawn — that is the whole point of
...
// `summarize()` + `tomtom-analyse-data` and deletes this module wholesale.
// ============================================================================
...
export function trimEVSearchResponse<T extends { features?: Array<Record<string, unknown>> }>(
  ...
  trimmed.features?.forEach((feature) => {
    ...
    // reachable via tomtom-describe-dataset / tomtom-analyse-data on the
    // dataset_id, which is where the untrimmed payload now lives.
