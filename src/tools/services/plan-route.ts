...
export async function findReachableAreasHandler(
  ...
  try {
    ...
    // Flatten every returned polygon into one FeatureCollection so the dataset is
    // a single searchable shape set — which is what makes it usable as a
    // `where.dataset_ids` scope for a follow-up search.
    ...
    return ok(
      {
        ...
        ...(shown < total && {
          ...
          truncationNote:
            ...
            "answerable from this list. Totals, counts and per-road breakdowns are NOT: run " +
            `tomtom-analyse-data over dataset ${dataset.id} for anything covering all ${total}.`,
        }),
        _meta: datasetMeta(dataset, show_ui),
      },
