...
describe("createRoutingTools", () => {
  it("should register all 3 routing tools without backend metadata", async () => {
    ...
    // Each tool must have schema and description, and no backend tag
    for (const call of mockRegisterAppTool.mock.calls) {
      ...
      expect(options._meta).not.toHaveProperty("backend");
    }
  });
});
