...
vi.mock("../handlers/mapOrbisHandler", () => ({
  createDynamicOrbisMapHandler: vi.fn(() => vi.fn()),
}));

const { createMapOrbisTools } = await import("./mapOrbisTools");

describe("createMapOrbisTools", () => {
  it("should register tomtom-dynamic-map with Orbis backend metadata and schema", async () => {
    const mockServer = {} as McpServer;
    await createMapOrbisTools(mockServer);
