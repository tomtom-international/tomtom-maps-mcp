...
export function createRoutingTools(server: McpServer): void {
  ...
  server.registerTool(
    "tomtom-waypoint-routing",
    {
      ...
      description:
        "Plan multi-stop routes through 3 or more waypoints. Use when the user needs to visit multiple locations in sequence (e.g. 'route from A to B via C and D'). Returns optimized turn-by-turn directions, total distance and travel time. Route polylines are omitted unless response_detail is 'full'. For simple A-to-B routes, use tomtom-routing instead.",
