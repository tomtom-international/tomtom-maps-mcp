...
export function createRoutingTools(server: McpServer): void {
  ...
  server.registerTool(
    "tomtom-routing",
    {
      ...
      description:
        "Calculate optimal routes between two locations. The primary tool for directions, routes, travel time, or distance between places (e.g. 'route from Amsterdam to Berlin', 'how long to drive from A to B'). Returns distance, travel time and traffic delay. Turn-by-turn instructions require instructionsType and response_detail 'full'. Route polylines are omitted unless response_detail is 'geometry'. Multi-stop routes with 3+ waypoints are handled by tomtom-waypoint-routing; visualizing multiple routes or combining routes with markers/polygons on a single map image is handled by tomtom-dynamic-map.",
  ...
  server.registerTool(
    "tomtom-waypoint-routing",
    {
      ...
      description:
        "Plan multi-stop routes through 3 or more waypoints. Use when the user needs to visit multiple locations in sequence (e.g. 'route from A to B via C and D'). Returns total distance and travel time, with a summary per leg. Turn-by-turn instructions require instructionsType and response_detail 'full'. Route polylines are omitted unless response_detail is 'geometry'. For simple A-to-B routes, use tomtom-routing instead.",
