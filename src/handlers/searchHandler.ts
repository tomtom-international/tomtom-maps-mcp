...
export function createAreaSearchHandler() {
  return async (params: AreaSearchParams) => {
    ...
    try {
      ...
      const result = await searchInArea(searchParams as unknown as AreaSearchOptions);

      const boundary = buildSearchBoundaryFeature(searchParams as unknown as AreaSearchOptions);
