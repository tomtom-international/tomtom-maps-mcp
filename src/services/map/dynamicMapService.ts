...
// Web Mercator world width at zoom 0, in pixels (the 256-px tile convention).
const TILE_SIZE = 256;
...
/**
 * Builds the state the dynamic-map MCP app renders from: the basemap style,
 * the initial viewport, and GeoJSON sources and layers for the markers, drawn
 * lines, polygons and calculated route plans.
 *
 * Route plans are calculated through the Routing API; each plan's outcome
 * (distance and travel time, or the reason it could not be calculated) is
 * returned alongside the state so callers can describe the map in text.
 */
export async function buildDynamicMap(options: DynamicMapOptions): Promise<DynamicMapResponse> {
  validateApiKey();
  ...
  try {
    ...
    const routePlanOutcomes: RoutePlanOutcome[] = [];
    if (isRoutePlanningMode) {
      for (let planIdx = 0; planIdx < routePlans.length; planIdx++) {
        ...
        const outcome: RoutePlanOutcome = {
          label: planLabel,
          travelMode: plan.travelMode || "car",
          waypointCount: plan.waypoints?.length ?? 0,
          ...(plan.origin?.label && { originLabel: plan.origin.label }),
          ...(plan.destination?.label && { destinationLabel: plan.destination.label }),
        };
        routePlanOutcomes.push(outcome);
        ...
        try {
          ...
          if (!originCoords || !destCoords) {
            ...
            outcome.error = "Invalid origin or destination coordinates";
            continue;
          }
          ...
          const firstSummary = routeResult?.routes?.[0]?.summary;
          if (firstSummary) {
            outcome.lengthInMeters = firstSummary.lengthInMeters || 0;
            outcome.travelTimeInSeconds = firstSummary.travelTimeInSeconds || 0;
            outcome.trafficDelayInSeconds = firstSummary.trafficDelayInSeconds || 0;
          } else {
            outcome.error = "The Routing API returned no route";
          }
          ...
          outcome.error = routeError instanceof Error ? routeError.message : String(routeError);
          logger.warn(
            { planIdx, label: planLabel, error: outcome.error },
    ...
    // Snap to a whole zoom level within the supported range
    ...
    // ── Return response ──────────────────────────────────────────────────
    logger.info(
      { width, height, zoom, sources: Object.keys(mapStateSources).length },
    ...
    return {
      ...
      mapState,
      summary: {
        markers: markerFeatures.length,
        polygons: polygonFeatures.length,
        lines: isRoutePlanningMode ? 0 : routeFeatures.length,
        ignoredLines: isRoutePlanningMode ? (directRoutes?.length ?? 0) : 0,
        routePlans: routePlanOutcomes,
      },
    };
  } catch (error: unknown) {
