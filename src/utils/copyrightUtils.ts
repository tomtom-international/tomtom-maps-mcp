...
export async function fetchCopyrightCaption(useOrbis: boolean): Promise<string> {
  try {
    ...
    const response = await tomtomClient.get<{ copyrightsCaption?: string }>(copyrightUrl, {
      responseType: "json",
      params: requestParams,
    });

    if (response.data?.copyrightsCaption) {
    ...
    logger.warn({ error: errorMessage }, "Failed to fetch copyright caption. Using fallback.");
    // Fallback to static text if API call fails
    return useOrbis ? "©TomTom, ©OpenStreetMap" : "©TomTom";
  }
}
